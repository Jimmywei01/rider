  ...
  <el-dialog
    ...
    <el-form
      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="數量" prop='numbers' :label-width="formLabelWidth">
            <el-input v-model="form.numbers" placeholder="請輸入數量"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
...
/**
  ...
  3.打 api
*/
import { validateMoto, validateMotoCatalog, validateCc, validateNumber } from '@/utils/vaildate.js'
  ...
  */
  methods: {
    ...
    submitEdit (form) {
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          this.$store.dispatch('motoHouse/updateMoto', this.form)
            .then(res => {
              this.$emit('update:updateLoading', false)
              this.$swal.fire({
                icon: 'success',
